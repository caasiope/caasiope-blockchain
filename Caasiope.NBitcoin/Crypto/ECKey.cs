using System;
using Caasiope.NBitcoin.BouncyCastle.asn1.x9;
using Caasiope.NBitcoin.BouncyCastle.crypto.ec;
using Caasiope.NBitcoin.BouncyCastle.crypto.parameters;
using Caasiope.NBitcoin.BouncyCastle.crypto.signers;
using Caasiope.NBitcoin.BouncyCastle.math;
using Caasiope.NBitcoin.BouncyCastle.math.ec;
using Caasiope.NBitcoin.BouncyCastle.math.ec.custom.sec;

namespace Caasiope.NBitcoin.Crypto
{
    public class ECDSAKey
    {
        private readonly ECKey key;

        public ECDSAKey(byte[] vch, bool isPrivateKey)
        {
            key = new ECKey(vch, isPrivateKey);
        }

        public bool Verify(byte[] bytes, ECDSASignature sig)
        {
        ...
        {
            return key.GetPubKey(isCompressed);
        }
    }
	...
	internal class ECKey
	{
        ...
        public ECDSASignature Sign(Hash256 hash)
		{
		    return Sign(hash.ToBytes());
		}
        ...
        internal bool Verify(Hash256 hash, ECDSASignature sig)
		{
		    return Verify(hash.ToBytes(), sig);
		}

		internal bool Verify(byte[] hash, ECDSASignature sig)
		{
        ...
        public static ECKey RecoverFromSignature(int recId, ECDSASignature sig, Hash256 message, bool compressed)
		{
			if(recId < 0)
